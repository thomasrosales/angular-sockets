...
class AbstractValidation:
    ...
    def pre_validation(self, *args, **kwargs) -> None:
        """
        Metodo abstracto que puede ser implementado para realizar validaciones
        pre creacion del objeto
        """
        pass
    ...
    def post_validation(self, *args, **kwargs) -> None:
        """
        Metodo abstracto que puede ser implementado para realizar validaciones
        luego de la persistencia del objeto en BD
        """
        pass
    ...
    def validation(self, *args, **kwargs) -> None:
        """
        Metodo abstracto que puede ser implementado para realizar validaciones
        luego de la creacion del objeto
        """
        pass
...
class GenericRetrieveResponse:
    ...
    @error_raise_handler
    def retrieve(self, id: int) -> Tuple[Dict, int]:
...
class GenericCreateResponse(AbstractValidation):
    ...
    def pre_create(self, *args, **kwargs) -> Dict:
        """
        Devuelve el body del request serializado y pre-validado
        """
        try:
            if not self.data:
    ...
    def create(self, *args, **kwargs) -> Tuple[Dict, int]:
        ...
        self.serializer = self.pre_create(*args, **kwargs)

        self.instance = self.create_instance(*args, **kwargs)

        self.validation(*args, **kwargs)

        deserializer = self.post_create(self.instance)

        return deserializer, HTTPStatus.CREATED

    @error_raise_handler
    def create_instance(self, *args, **kwargs) -> Type[model]:
        """
        Instancia el objeto basado en self.model
        """
    ...
    def post_create(self, instance: Type[model]) -> Dict:
        """
        Persiste el objeto instanciado en BD y ejecuta una post-validacion

        Devuelve: Objeto instanciado des-serializado
        """
